      ...
      module Socks5
        ...
        class ServerClient
          ...
          def handle_authentication_creds(request)
            ...
            raise "Invalid SOCKS5 authentication packet received." unless version.unpack1('C') == 0x01

            username_length = @lsock.read(1).unpack1('C')
            username        = @lsock.read(username_length)

            password_length = @lsock.read(1).unpack1('C')
          ...
          def handle_command_udp_associate(_request)
            response              = ResponsePacket.new
          ...
          def stop
            @mutex.synchronize do
              unless @closed
                ...
                @client_thread.kill if @client_thread&.alive?

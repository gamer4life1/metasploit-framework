  ...
  module Proto
    module Http
      class ClientRequest
        ...
        DefaultUserAgent = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)".freeze
        DefaultConfig = {
          #
          # Regular HTTP stuff
          #
          ...
          if opts['cgi']
            ...
            if opts['pad_get_params']
              1.upto(opts['pad_get_params_count'].to_i) do |_i|
                qstr << '&' if !qstr.empty?
                qstr << set_encode_uri(Rex::Text.rand_text_alphanumeric(rand(1..32)))
                qstr << '='
                qstr << set_encode_uri(Rex::Text.rand_text_alphanumeric(rand(1..32)))
              end
            end
            ...
            if opts['pad_post_params']
              1.upto(opts['pad_post_params_count'].to_i) do |_i|
                rand_var = Rex::Text.rand_text_alphanumeric(rand(1..32))
                rand_val = Rex::Text.rand_text_alphanumeric(rand(1..32))
                pstr << '&' if !pstr.empty?
      ...
      protected
        ...
        def set_uri
          ...
          if opts['uri_dir_fake_relative']
            ...
            uri_str.split('/', -1).each do |part|
              cnt = rand(2..9)
              1.upto(cnt) do |_idx|
                buf << "/" + Rex::Text.rand_text_alphanumeric(rand(1..32))
              end
              buf << ("/.." * cnt)
              buf << "/" + part
            end
        ...
        def set_method
          ...
          if opts['method_random_invalid']
            ret = Rex::Text.rand_text_alpha(rand(1..20))
          end
        ...
        def set_version
          ...
          if opts['version_random_invalid']
            ret = Rex::Text.rand_text_alphanumeric(rand(1..20))
          end
          ...
          if opts['pad_fake_headers']
            1.upto(opts['pad_fake_headers_count'].to_i) do |_i|
              buf << set_formatted_header(
                Rex::Text.rand_text_alphanumeric(rand(1..32)),
                Rex::Text.rand_text_alphanumeric(rand(1..32))
              )
            end
          end

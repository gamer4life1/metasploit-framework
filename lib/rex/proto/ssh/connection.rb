    ...
    module Ssh
      ...
      module AccessControlList
        ...
        def deny=(host, port, bind = false)
          if @acl
            @acl[bind ? :bind : :connect].reject! do |ent|
              ent == "#{host}:#{port}"
            end
          end
        end
        ...
        def permit?(host, port, bind = false)
          @acl && ["#{host}:#{port}", "*:*", "#{host}:*", "*:#{port}"].any? do |m|
      ...
      class Connection < ::HrrRbSsh::Connection
        ...
        def self.default_options
          noneauth = HrrRbSsh::Authentication::Authenticator.new { |_context| true }
          return {
        ...
        def initialize(io = nil, options = default_options, context = {})
          @context = context
        ...
        def open_channel_keys(ctype = 'session')
          channels.keys.sort.select do |cn|
            (channels[cn].closed? === false) && (
              ctype.nil? || (channels[cn].channel_type == ctype)
            )
          end
        end
        ...
        def close
          super
          @transport.io.close if @transport && !@transport.io.closed?
        end
      ...
      class ChannelRelay
        ...
        def initialize(src, dst, _threadname = "SshChannelMonitorRemote")
      ...
      module IOMergeAbstraction
        ...
        def has_read_data?(timeout = nil)
          ...
          begin
            if ((rv = ::IO.select([ fd_rd ], nil, nil, timeout)) &&
                (rv[0]) &&
                (rv[0][0] == fd_rd))
              true
            else
        ...
        def close
          fd_rd.close if (fd_rd && !fd_rd.closed?)
          fd_wr.close if (fd_wr && !fd_wr.closed?)
        end

        def closed?
          (fd_rd.nil? || fd_rd.closed?) && (fd_wr.nil? || fd_wr.closed?)
        end
      end
      ...
      class ChannelFD
        ...
        def initialize(parent, _chan_id = nil)
          @parent = parent
        end
        ...
        def cid=(chan_id)
          if @parent.connection.open_channel_keys.include?(chan_id)
            ...
            raise "Invalid Channel ID passed to #{inspect}"
          end
        end
        attr_reader :parent

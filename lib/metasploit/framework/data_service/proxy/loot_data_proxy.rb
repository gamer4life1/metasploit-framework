module LootDataProxy
  def report_loot(opts)
    ...
    data_service_operation do |data_service|
      if !data_service.is_a?(Msf::DBManager)
        opts[:data] = Base64.urlsafe_encode64(opts[:data].empty? ? "" : opts[:data].join('')) if opts[:data]&.is_a?(Array) else opts[:data]
      end
      add_opts_workspace(opts)
      data_service.report_loot(opts)
    end
  rescue StandardError => e
    log_error(e, "Problem reporting loot")
  ...
  def find_or_create_loot(opts)
    ...
    log_error(e, "Problem finding or creating loot")
  ...
  def loots(opts = {})
    ...
    log_error(e, "Problem retrieving loot")
  ...
  def update_loot(opts)
    ...
    log_error(e, "Problem updating loot")

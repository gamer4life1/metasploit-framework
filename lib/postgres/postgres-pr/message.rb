    ...
    module PostgresPR
      ...
      class Message
        ...
        def self.read(stream, startup = false)
          ...
          length = stream.read_exactly_n_bytes(4).to_s.unpack1('N') # FIXME: length should be signed, not unsigned
      ...
      module NoticeErrorMixin
        ...
        def parse(buffer)
          super do
            ...
            @field_values = []
            @field_values << buffer.read_cstring while buffer.position < buffer.size - 1

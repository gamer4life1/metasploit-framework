  ...
  module Util
    ...
    class DotNetDeserialization
      ...
      def self.generate(cmd, gadget_chain: DEFAULT_GADGET_CHAIN, formatter: DEFAULT_FORMATTER)
        serialized = generate_gadget_chain(cmd, gadget_chain: gadget_chain)
        serialized = generate_formatted(serialized, formatter: formatter) unless formatter.nil?
        serialized
      end

...
module Msf
  module RPC
    class RPC_Module < RPC_Base
      ...
      def rpc_evasion
        { "modules" => framework.evasion.keys }
      end
      ...
      def rpc_payloads(module_info = nil, arch = nil)
        ...
        framework.payloads.each_module('Arch' => arch_filter) do |name, mod|
          if module_info.nil?
            data << name
          else
            ...
            data[name] = tmp_mod_info.select { |k, _v| module_info.include?(k) }
          end
        end
      ...
      def rpc_encoders(module_info = nil, arch = nil)
        ...
        data = module_info.nil? ? [] : {}
        arch_filter = !arch.nil? && !arch.empty? ? arch : nil
        framework.encoders.each_module('Arch' => arch_filter) do |name, mod|
          if module_info.nil?
            data << name
          else
      ...
      def rpc_nops(module_info = nil, arch = nil)
        ...
        data = module_info.nil? ? [] : {}
        arch_filter = !arch.nil? && !arch.empty? ? arch : nil
        framework.nops.each_module('Arch' => arch_filter) do |name, mod|
          if module_info.nil?
            data << name
          else
            tmp_mod_info = ::JSON.parse(Msf::Serializer::Json.dump_module(mod.new), symbolize_names: true)
      ...
      def rpc_info(mtype, mname)
        ...
        res['arch'] = m.arch.map(&:to_s)
        res['platform'] = m.platform.platforms.map(&:to_s)
        res['authors'] = m.author.map(&:to_s)
      ...
      def rpc_search(match)
        matches = []
        framework.search(match).each do |m|
      ...
      def rpc_running_stats
        {
          "ready" => framework.ready.size,
          "running" => framework.running.size,
          "results" => framework.results.size
        }
      end
      ...
      def rpc_results(uuid)
        ...
        elsif framework.running.include? uuid
          { "status" => "running" }
        elsif framework.ready.include? uuid
          { "status" => "ready" }
        else
      ...
      def rpc_ack(uuid)
        { "success" => !!framework.results.delete(uuid) }
      end
      ...
      def _find_module(mtype, mname)
        ...
        if mname !~ %r{^(exploit|payload|nop|encoder|auxiliary|post|evasion)/}
          mname = mtype + "/" + mname
        end

        mod = framework.modules.create(mname)

        error(500, "Invalid Module") if !mod
        mod
      end

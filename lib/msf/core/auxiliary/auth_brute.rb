  ...
  module Auxiliary::AuthBrute
    def initialize(info = {})
      ...
      register_advanced_options([
                                  ...
                                  OptInt.new('MaxGuessesPerUser', [ false, '
    ...
    def extract_word_pair_from_memory(memloc)
      ...
      if obj.all_creds.empty?
        these_creds = obj.builders.select { |x| x.respond_to? :imported_users }.map(&:imported_users).flatten
      else
    ...
    def tried_over_total(ip, port)
      ...
      pad = total.to_s.size
      format("[%0#{pad}d/%0#{pad}d] - ", current_try, total)
    end
